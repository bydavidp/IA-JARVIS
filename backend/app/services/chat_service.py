...
CHAT_FALLBACK_SENTINEL = "__CHAT_FALLBACK__"
...
class ChatService:
    ...
    async def chat(
        ...
        Primero intenta enrutar como acción del agente.
        Si es conversación, usa Ollama (flujo original).
        """
        # Intentar enrutamiento como acción del agente
        tokens = []
        async for token in agent_router.route(message, mode):
            if token == CHAT_FALLBACK_SENTINEL:
                break
            tokens.append(token)

        # Si el router produjo respuesta, devolverla
        if tokens:
            for t in tokens:
                yield t
            return

        # Fallback: chat normal con Ollama
